...
Optimized for memory efficiency and cloud-safe execution.
"""
...
def multi_scale_detect_and_match(
    ...
    Early-exits when 1.0 scale yields robust inlier candidates to save CPU and RAM.
    """
    ...
    # Check 1.0 scale first
    ordered_scales = [1.0] + [s for s in scales if s != 1.0]
    
    for s in ordered_scales:
        if s > 1.0 and max(image_b.shape) * s > 1400:
            # Skip excessive upscaling on cloud containers to protect RAM
            continue
        ...
        # If scale 1.0 produces sufficient candidates, skip remaining scale octaves
        if s == 1.0 and len(good_matches) >= max(50, config.min_candidate_matches * 2):
            break
